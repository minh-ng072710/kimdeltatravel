...
class ATDTP_Shortcodes_Visual_Composer
{
	...
	public function init() {
		...
		if ( is_admin() ) {
			...
			_atdtp_define_visual_composer_container_classes();
	...
	public function action_vc_after_init() {
		$adventure_icon_selector_param = array(
			'type' => 'iconpicker',
			'weight' => 1,
		...
		$icon_set_title = 'Adventure Tours';
		...
		if ( $type_param ) {
			$type_param['value'][$icon_set_title] = 'adventuretours';
			$type_param['weight'] = 100;

			vc_add_params( 'vc_icon', array(
				$type_param,
				$adventure_icon_selector_param
			) );
		}

		// Integrates icons selector of Adventure Tours icons into Visual Composer [vc_tta_section] shortcode.
		$icon_params_prefix = 'i_';
		$icon_type_param = WPBMap::getParam( 'vc_tta_section', $icon_params_prefix . 'type' );
		if ( $icon_type_param ) {
			$icon_type_param['value'][$icon_set_title] = 'adventuretours';

			$overrides = array(
				'param_name' => $icon_params_prefix . $adventure_icon_selector_param['param_name'],
				'integrated_shortcode' => 'vc_icon',
				'integrated_shortcode_field' => $icon_params_prefix,
				'weight' => -99,
			);
			$overrides['dependency'] = $adventure_icon_selector_param['dependency'];
			$overrides['dependency']['element'] = $icon_params_prefix . $overrides['dependency']['element'];

			// Updates 'weight' for Element class parameter to move it to the bottom
			// of params list.
			$el_class_param = WPBMap::getParam( 'vc_tta_section', 'el_class' );
			$el_class_param['weight'] = -100;

			vc_add_params( 'vc_tta_section', array(
				$icon_type_param,
				// Overrides some values of the icon selector.
				$overrides + $adventure_icon_selector_param,
				$el_class_param
			) );
		}
	}
...
function _atdtp_define_visual_composer_container_classes(){
