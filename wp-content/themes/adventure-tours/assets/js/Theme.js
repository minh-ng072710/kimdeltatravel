...
var Theme = {
	...
	initSharrres: function(config){
		...
		var elements = jQuery(config.itemsSelector);
		...
		var initSharreBtn = function(){
			var el = jQuery(this),
				...
				curConf = {
					urlCurl: config.urlCurl ? config.urlCurl : '',
					enableHover: false,
					enableTracking: true,
...
Theme.tourBookingForm = {
			...
			jQuery.ajax( {
				...
				complete:function(response, status){
					...
					if ( is_success ) {
						...
						self._renderFormErrors(
							form,
							r.data && r.data.errors ? r.data.errors : {
	...
	_resetTimeFieldOptions: function( tfield ){
		if ( tfield ) tfield.find('option').remove();
	},
	...
	_renderPriceDetails:function( config ){
		if ( ! this.renderPriceDetails ) return;
		...
		var self = this,
			...
			renderResult = function( is_valid, html, validationErrors ) {
				price_element.html( html ? html : '' );
				self._renderFormErrors(form, validationErrors);
				if ( is_valid ) {
		...
		if ( this.itemsDataPriceUrl ) {
			...
			this._getFormPriceItemsData(
				...
				function( response ) {
					renderResult(
						...
						response && response.as_html ? response.as_html : '',
						response && response.data && response.data.errors ? response.data.errors : null
					);
				},
		...
		} else { // Client side price calculation logic.
			...
			if ( explanation_parts.length > 0 ) {
				...
				renderResult( true, explanation_parts.join('<br>') );
			} else {
				renderResult( false, '' );
			}
		}
	...
	_renderFormErrors:function(form, formErrors){
		var errorClass = 'form-validation-item',
			renderFieldErrors = function(el, errors) {
				// Errors rendeing over the select elements wrapped with `selectpicker` component.
				if ( el.is('select:hidden') ){
					var sPickerBtn = el.parent().find('button.selectpicker');
					if ( 0<sPickerBtn.length ) {
						el = sPickerBtn;
					}
				}
				if ( errors ) {
					// Forces "re-render" new error message, if the old one has not been hidden yet.
					if ( el.data('bs.tooltip') ) el.tooltip('destroy');
					el.attr('title', errors.join(' '))
						.addClass(errorClass);
				} else if ( el.hasClass(errorClass) ) {
					el.removeAttr('title')
						//.attr('data-original-title', '')
		...
		var renderedErrorKeys = [],
			generalErrors = [],
			allElements = jQuery(form[0].elements),
			elements = allElements.filter('[name],[data-fieldkey]').each(function(){
				var el = jQuery(this),
					field_key = el.data('fieldkey') || el.attr('name');

				// If element has "hidden" type - would render error above the booking button.
				if ( field_key && 'hidden' != el.attr('type') ) {
					if ( field_key.indexOf('[') > 0 ) {
						field_key = field_key.replace(/\w+\[(\w+)\]/, '$1');
					}
					renderedErrorKeys.push(field_key);
					renderFieldErrors( el, formErrors && formErrors[field_key] ? formErrors[field_key] : null );
				}
			});

		if ( formErrors ){
			for ( var _fieldKey in formErrors ) {
				if ( 0 > renderedErrorKeys.indexOf(_fieldKey) && 0 < formErrors[_fieldKey].length ){
					generalErrors = generalErrors.concat(formErrors[_fieldKey]);
				}
			}
		}
		...
		setTimeout(function(){
			Theme.FormValidationHelper
				.initTooltip( allElements.filter('.' + errorClass) )
				.tooltip('show');
		}, 200);

		if ( 0 < generalErrors.length ) {
			this._getGeneralErrorsBlock(form, true).show().html(generalErrors.join('<br>'));
		} else {
			this._getGeneralErrorsBlock(form, false).hide().html('');
		}
	},

	/**
	 * Returns jQuery object for element that should be used as a container
	 * for error messages that are not related with any specific form tield.
	 *
	 * @return jQuery object
	 */
	_getGeneralErrorsBlock:function(form, forceCreation){
		var generalErrorsContainer = form.find('[data-role=errors-block]');
		if (forceCreation && 1 > generalErrorsContainer.length){
			return jQuery('<div class="form-block__errors-block" data-role="errors-block"></div>')
				.insertBefore(form.find('input').last());
		}
		return generalErrorsContainer;
	},
