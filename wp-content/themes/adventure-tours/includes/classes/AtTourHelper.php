...
class AtTourHelper
{
	...
	public static function get_tours_page_id( $in_default_language = false, $validate = false, $reset_cache = false ) {
		...
		$lang_cache_key = $in_default_language || self::_is_default_lang() ? '_' : self::_current_lang();
		if ( !$lang_cache_key ) $lang_cache_key = '_';
			...
			if ( $validate && isset( $cache[ $unvalidated_cache_key ] ) ) {
				...
				if ( self::_is_wpml_in_use() ) {
					...
					if ( $in_default_language && !self::_is_default_lang() ){
						$decoded = maybe_unserialize( apply_filters('wpml_unfiltered_admin_string', null, VP_OPTION_KEY) );
					} else {
	...
	protected static function _default_lang(){
		return self::_is_wpml_in_use() ? apply_filters( 'wpml_default_language', null ) : null;
	}

	protected static function _current_lang(){
		return self::_is_wpml_in_use() ? apply_filters( 'wpml_current_language', null ) : null;
		// return self::_is_wpml_in_use() ? ICL_LANGUAGE_CODE : null;
	}

	protected static function _is_default_lang(){
		return ! self::_is_wpml_in_use() || self::_default_lang() == self::_current_lang();
	}
	#endregion
}
