...
class WC_Report_ADT_General extends WC_Report_ADT_Base
{
	...
	public function get_main_chart() {
		...
		if ( $filter_by_ticket_dates ) {
			...
			$mysql_timestapm_ranges = $wpdb->get_row(
				sprintf('select UNIX_TIMESTAMP("%s") as start_date, UNIX_TIMESTAMP("%s") as end_date',
		...
		$group_translated_products = adventure_tours_check( 'is_wpml_in_use' )
			&& apply_filters( 'adventure_tours_reports_group_translated_tours', false );

		foreach ( $raw_rows as $row ) {
			$pid = $row->product_id;
			$real_pid = $group_translated_products
				? apply_filters( 'wpml_object_id', $pid, 'product', true, ICL_LANGUAGE_CODE )
				: $pid;
			...
			$row->order_status_label = isset( $stuses_list[ $row->order_status ] )
				? $stuses_list[ $row->order_status ] 
				: $row->order_status;
			$row->booking_date_formatted = $booking_form
				? $booking_form->convert_date_for_human( $row->date )
				: $row->date;
			...
			if ( $order_ids ) {
				foreach ( $order_ids as $order_id) {
					...
					if( $cur_order ) {
						$billing_email = $is_wc_older_than_30
							? $cur_order->billing_email
							: $cur_order->get_billing_email();
						if ( $billing_email ) {
		...
		if ( $render_stock_column ) {
			...
			foreach ( $records as &$_r ) {
				if ( ! isset( $stock_cache[ $_r->product_id ] )) {
					if ( ! $expand_only_current_period ) {
						$_tour_stock = $stock_cache[ $_r->product_id ] = adventure_tours_di( 'tour_booking_service' )
							->get_expanded( $_r->product_id, $_expand_booking['start'], $_expand_booking['end'] );
					} else {
