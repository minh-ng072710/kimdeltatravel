...
<form class="variations_form cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype="multipart/form-data" data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( wp_json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php echo esc_html( apply_filters( 'woocommerce_out_of_stock_message', __( 'This product is currently out of stock and unavailable.', 'adventure-tours' ) ) ); ?></p>
	<?php else : ?>
						...
						<td class="value">
							<?php
								...
								wc_dropdown_variation_attribute_options( array(
									...
									'product' => $product,
									// 'selected' => $selected,
									'class' => 'select2-selector'
								) );
								echo end( $attribute_keys ) === $attribute_name ? wp_kses_post( apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html__( 'Clear', 'adventure-tours' ) . '</a>' ) ) : '';
							?>
						</td>
					</tr>
				<?php endforeach;?>
		...
		<?php //do_action( 'woocommerce_before_add_to_cart_button' ); // since WooCommerce 3.4.0 ?>
		...
		<?php // do_action( 'woocommerce_after_add_to_cart_button' ); // since WooCommerce 3.4.0 ?>
	<?php endif; ?>
