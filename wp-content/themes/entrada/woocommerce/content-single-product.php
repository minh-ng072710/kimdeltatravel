...
if (has_post_thumbnail(get_the_ID())) :
    ...
    $featured_img = wp_get_attachment_image_src($thumb_id, 'single-post-thumbnail');
    $thumb = wp_get_attachment_image_src($thumb_id, 'medium');

    $gallery_image_urls[$thumb_id] = $featured_img[0];
...
if ($attachment_ids) {
    foreach ($attachment_ids as $attachment_id) {
        $gallery_image_urls[$attachment_id] = wp_get_attachment_url($attachment_id, 'large');
    }
}
...
if ($entrada_product_type == 'shop_item') :
    ...
    <div itemscope itemtype="http://schema.org/Product">
            ...
            <div class="two-columns row">
                <div class="col col-md-6">
                    ...
                    <?php if ($gallery_image_urls) { ?>
                        <div id="tour-slide">
                            <?php
                            $entrada_product_img_resize_id = get_option('entrada_product_img_resize_id');
                            $image_url = '';
                            foreach ($gallery_image_urls as $key => $slider_img) {

                                if (isset($entrada_product_img_resize_id) && $entrada_product_img_resize_id == 'wp_default') {
                                    $image_url = $slider_img;
                                    if (!empty($image_url)) {
                                        ?>
                                        <div class="slide">
                                            <div class="bg-no-stretch">
                                                <img src="<?php echo esc_url($image_url); ?>" itemprop="image" alt="<?php echo entrada_image_attributes_from_src($slider_img, 'title', true); ?>" height="1149" width="960">
                                            </div>
                                        </div>
                                        <?php
                                    }
        ...
        <!--								<strong <?php //echo entrada_price_schema_micro_data_link(get_the_ID());          ?>><?php //entrada_product_price(get_the_ID(), true);          ?></strong>-->
                                <?php if (get_field('gia_ghep_doan')):
                                    ?>
                                    <strong><?php echo pll__('Giá ghép đoàn'); ?>: <?php
                                        $myNumber = get_field('gia_ghep_doan');
                                        echo number_format($myNumber);
                                        ?> đ</strong>
                                <?php endif; ?>
                                <?php if (get_field('gia_tour_rieng_tu')):
                                    ?>
                                    <strong><?php echo pll__('Giá tour nhóm lẻ'); ?>: <?php
                                        $myNumber = get_field('gia_tour_rieng_tu');
                                        // $format_number_1 = number_format(the_field('gia_tour_rieng_tu'));
                            ...
                            <div class="description" itemprop="description">
                                <?php //the_content();  ?>
                                <div><?php echo pll__('Thời gian'); ?>: <strong><?php the_field('thoi_gian') ?></strong></div>
                                <div><?php echo pll__('Phương tiện'); ?>: <strong><?php the_field('phuong_tien') ?></strong></div>

                                <div><?php echo pll__('Điểm xuất phát'); ?>: <strong><?php the_field('diem_xuat_phat') ?></strong></div>

                                <div><?php echo pll__('Điểm đến'); ?>: <strong><?php the_field('diem_den') ?></strong></div>
                            ...
                            <ul class="reviews-info">
                                <li itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
                                    <div class="info-left">
                                        ...
                                        <span class="value" itemprop="reviewCount"><?php
                                            $total_reviews = entrada_post_total_reviews(get_the_ID(), true);
                                            printf(esc_html(_n('%d Review', '%d Reviews', $total_reviews, 'entrada')), $total_reviews);
                                            ?></span>
                                    </div>
                                ...
                                <li>
                                    <div class="info-left">
                                        ...
                                        <span class="value"><?php _e('Holiday Type', 'entrada'); ?></span>
                                    </div>
                                    <div class="info-right">
                                        <?php
                                        /* Product Tag..... */
                            ...
                            <div class="btn-holder">
                                <form class="cart" method="post" enctype="multipart/form-data" action="/dat-tour/">
                                    ...
                                    <input type="hidden" name="ghep-doan" value="Giá ghép đoàn: <?php echo number_format(get_field('gia_ghep_doan')); ?> đ">
                                    <input type="hidden" name="rieng-tu" value="Giá tour nhóm lẻ: <?php echo number_format(get_field('gia_tour_rieng_tu')); ?> đ">
                                    <input type="hidden" name="thoi-gian" value="THỜI GIAN: <?php the_field('thoi_gian'); ?>">
                                    <input type="hidden" name="phuong-tien" value="PHƯƠNG TIỆN: <?php the_field('phuong_tien'); ?>">
                                    <input type="hidden" name="diem-xuat-phat" value="ĐIỂM XUẤT PHÁT: <?php the_field('diem_xuat_phat'); ?>">
                                    <input type="hidden" name="diem-den" value="ĐIỂM ĐẾN: <?php the_field('diem_den') ?>">
                                    ...
                                    if ($product->is_type('external')) {
                                        ...
                                        if ($variations) {
                                            echo '<a href="#tab06" class="btn btn-lg btn-info book_now_dates">' . pll__('Đặt ngay') . '</a>';
                                        } else {

                                            echo '<button type="submit" class="btn btn-lg btn-info" id="book_now_dates">' . pll__('Đặt ngay') . '</button>';
                                        }
                                    } else {
                                        echo '<button type="submit" class="btn btn-lg btn-info" id="book_now_dates">' . pll__('Đặt ngay') . '</button>';
                                    }
                ...
                <div class="container">
                    ...
                    <ul class="nav nav-tabs text-center tab-add" role="tablist">
                        ...
                        if (isset($product_overview_detail) && !empty($product_overview_detail)) {
                            ...
                            ?>
                            <li role="presentation" class="active"><a href="#tab01" aria-controls="tab01" role="tab" data-toggle="tab"><?php echo pll__('Chương trình tour'); ?></a></li>
                            <?php
                        ...
                        if (isset($product_accommodation) && !empty($product_accommodation)) {
                            ...
                            ?>
                            <li role="presentation" <?php echo esc_html($active_class); ?>><a href="#tab03" aria-controls="tab03" role="tab" data-toggle="tab"><?php echo pll__('Chính sách tour'); ?></a></li>
                            <?php
                            $assigned_class = 'assigned';
                        }
                        ...
                        if (isset($product_gallery_images) && !empty($product_gallery_images)) {
                            ...
                            ?>
                            <li role="presentation" <?php echo esc_html($active_class); ?>><a href="#tab05" aria-controls="tab05" role="tab" data-toggle="tab"><?php echo pll__('Hình ảnh'); ?></a></li>
                            <?php
                            $assigned_class = 'assigned';
                        }
                        if ($variations) {
...
//                                echo '</div>';
                ...
                <?php
                if (isset($product_itinerary_arr) && 'yes' == $product_itinerary_count) {
                    $active_class = ($assigned_class == 'assigned') ? '' : 'active';
                    $cnt = 0;
                    ?>
                                            ...
                                            <li class="<?php echo esc_attr($activ_mode); ?>">
                                                ...
                                                <div class="slide">
                                                    <div class="slide-holder">
                                                        <?php
                                                        $itinerary_desc = apply_filters('the_content', $itinerary['itinerary_desc']);
