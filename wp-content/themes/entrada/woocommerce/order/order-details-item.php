...
<tr class="<?php echo esc_attr(apply_filters('woocommerce_order_item_class', 'woocommerce-table__line-item order_item', $item, $order)); ?>">
	...
	<td class="woocommerce-table__product-name product-name">
		...
		if (!empty($customer_start_date) && $product->is_type('simple')) {
			$customer_start_date = maybe_unserialize($customer_start_date);
			if (isset($customer_start_date[0])) {
				echo '<p>' . __('Start Date', 'entrada') . ' : ' . $customer_start_date[0] . '</p>';
			}
		}
	...
	<td class="woocommerce-table__product-total product-total">
		<?php echo sprintf(__('%s', 'entrada'), $order->get_formatted_line_subtotal($item)); ?>
	</td>
